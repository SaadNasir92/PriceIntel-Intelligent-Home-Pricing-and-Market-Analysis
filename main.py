...
def main():
    ...
    # Only using minmax now. Commenting on this portion.
    ...
    logger.info(
        f"Feature engineering completed. Number of selected features: {len(selected_features)}"
    )

    # Train and evaluate GBR model with hyperparameter tuning
    best_model, cv_results = model_trainer.train_gbr_with_tuning(
        X_train_engineered, y_train
    )

    # Evaluate the best model
    evaluation_results = evaluator.evaluate_model(best_model, X_test_engineered, y_test)
    logger.info(f"Best GBR model evaluation results: {evaluation_results}")

    results = {"GBR_MinMax": evaluation_results}
    evaluator.summarize_results(results)

    # Plotting feature importance
    logger.info(f"Selected features for importance plot: {selected_features}")
    evaluator.plot_feature_importance(
        best_model, X_test_engineered, y_test, selected_features, "GBR_tuned"
    )

    # Plottimg learning curve
    evaluator.plot_learning_curve(best_model, X_train_engineered, y_train, "GBR_tuned")

    # Plot hyperparameter impact
    evaluator.plot_hyperparameter_impact(cv_results)

    # Saving the best model
    save_model(best_model, f"best_model_GBR_tuned_{sample_size}")

    # Plot predictions
    evaluator.plot_predictions(best_model, X_test_engineered, y_test)
...
if __name__ == "__main__":
    ...
    # Commenting out loop since we have decided the model/scaler we are focusing on.
