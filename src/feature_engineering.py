...
class FeatureEngineer:
    ...
    def engineer_features(self, X_train, X_test, y_train, feature_names):
        ...
        X_train_interactions = (
            self.create_interaction_terms(X_train, interaction_pairs)
        ...
        self.logger.info(
            f"X_train shape after interactions: {X_train_interactions.shape if X_train_interactions is not None else None}"
        )
        self.logger.info(
        ...
        X_train_important = (
            X_train[:, important_indices] if X_train is not None else None
        )
        ...
        self.logger.info(
            f"X_train shape after polynomial: {X_train_poly.shape if X_train_poly is not None else None}"
        )
        ...
        X_train_combined = (
            np.hstack([X_train, X_train_interactions, X_train_poly])
            if X_train is not None
            else None
        )
        ...
        if X_train_combined is not None and y_train is not None:
            X_train_selected, self.selected_feature_indices = (
                self.select_top_k_features(X_train_combined, y_train, k=50)
            )
        else:
            X_train_selected = None

        X_test_selected = (
            X_test_combined[:, self.selected_feature_indices]
            if self.selected_feature_indices is not None
            else X_test_combined
        )
