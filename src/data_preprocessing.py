...
class DataPreprocessor:
    ...
    def preprocess_data(self, filepath, target_column, scaler_type='robust', test_size=0.2, sample_size=None):
        ...
        onehot_columns = self.categorical_encoder.named_transformers_['onehot'].get_feature_names_out(categorical_columns)
        ...
        X_train_preprocessed, X_test_preprocessed = self.apply_preprocessing(X_train_encoded, X_test_encoded, scaler_type)
    ...
    def apply_scaling(self, X_train, X_test, scaler_type='robust'):
        X_train_scaled, X_test_scaled, scaler = self.scale_features(X_train, X_test, scaler_type)
        return X_train_scaled, X_test_scaled, scaler
    
    def apply_preprocessing(self, X_train, X_test, scaler_type):
        if scaler_type == 'robust':
            # Skip outlier handling for Robust Scaler
            X_train_scaled, X_test_scaled = self.scale_features(X_train, X_test, scaler_type)
        else:
